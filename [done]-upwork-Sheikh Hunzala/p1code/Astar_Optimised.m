...
% heuristic h defined as Euclidean distance between START and END
%h = sqrt( (Pend(1)-Pstart(1))^2 + (Pend(2)-Pstart(2))^2 );
...
while idxP ~= idxEnd
    ...
    % heuristic h defined as Euclidean distance between all positions and END
    [QtempR,QtempC] = ind2sub(sizeI,Qtemp);    % index to xy conversion
    h = sqrt( (QtempR-Pend(1)).^2 + (QtempC-Pend(2)).^2 );
    
    
    [~,idxMinWgt] = min(WeightsTemp + h);   % Index of the minimum weight remaining
    MinWgt = WeightsTemp(idxMinWgt);    % Value of the minimum weight                   
    ...
    %Weights(NeighborsQ) = min(Weights(NeighborsQ), NeighborsNewDist); 
%     length(Weights(Weights ~= Inf))
%     length(WeightsTemp(WeightsTemp ~= Inf))
    
    WeightsTemp(Qtemp == idxP) = [];
    Qtemp(Qtemp == idxP) = [];     % Altering x from Q
...
save('Path_Astar.mat','Previous','ShortPath','PathXY','Weights')
...
title(['A* shortest path | Arrival cost = ', num2str(Distance)], 'FontSize', 20);
hold off;